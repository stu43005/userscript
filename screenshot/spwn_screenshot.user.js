// ==UserScript==
// @name        Spwn.jp screenshot
// @description Add a button under the video to screenshot the video on Spwn.jp
// @namespace   ScreenshotTools
// @version     1.0.0
// @include     https://spwn.jp/*
// @include     https://virtual.spwn.jp/*
// @include     https://*.spwn.jp/*
// @run-at      document-end
// @noframes
// ==/UserScript==
...
spacerButton.className = "screenshotButton vjs-control";
spacerButton.style.width = "36px";
spacerButton.style.fontSize = "24px";
spacerButton.style.margin = "0 10px";
spacerButton.style.cssFloat = "left";
spacerButton.style.cursor = "pointer";
spacerButton.innerHTML = '📷';
spacerButton.onclick = captureScreenshot;

function addSpacerButton() {
    const spacerElem = document.getElementsByClassName("vjs-custom-control-spacer vjs-spacer")[0];
    if (spacerElem) {
        spacerElem.insertAdjacentElement('afterend', spacerButton);
    } else {
...
function addScreenshotButton() {
    const streamingOptionElem = document.getElementById("streamingOption");
    if (streamingOptionElem) {
        streamingOptionElem.insertAdjacentElement('beforeend', screenshotButton);
    } else {
...
function getFileName(player) {
    ...
    return `SPWN screenshot ${time}${ext}`;
}
